...
# ╔═╡ 7bd58b97-b4d3-46d3-b657-c82d9d918c72
begin
	# This is P^*(x) of f(x) which is a function that is proportional to the target distribution P(x)
	...
	# This is Q(x) or g(x)
	function proposalDistribution(θ, C0, X, priorGaussianProposal=false)
		if priorGaussianProposal
			return MvNormal(θ, C0)
		else
			return MvNormal(θ, Hermitian(inv(inv(C0) + (6/(π^2)) * X' * X)))
		end
	end

	# This is the function to solve a
	function aFunction()
		#Px = 
	end
end

# ╔═╡ d1c0605d-e609-4bea-aeba-ccdd4a8d3603
begin
	dim = 2+1
	x00 = zeros(dim)
	m0 = zeros(dim)
...
# ╔═╡ 107930c2-63c3-4c2a-b22f-6a522247a69f
begin
...
# ╔═╡ 333a8e1f-5050-4faf-b479-447032ef9952
mLP, VLP, _ = LaplaceLogisticR(D, targets, dim; m0=m0, V0 = C0);

# ╔═╡ 82aa8b4b-65d1-4fc7-9984-611b94a8a8d5
begin
	# What do we need to approximate a posterior using MCMC?
	# We need a function which can tell us the value of the posterior at a given point
	# We need a proposal distribution
	
	# First random state
	function metropolis(priorGaussianProposal=false)
		iters = 10
		x₀ = rand(proposalDistribution(mLP,C0,D, priorGaussianProposal))
		x_samples = Matrix(undef, 3, iters)
		xcur = x₀
		for i in 1:iters
			x_c =  rand(proposalDistribution(xcur,C0,D, priorGaussianProposal))
			p_x,_,_ = logPosteriorLogisticR(xcur, m0, C0, D, targets)
			p_xc,_,_ = logPosteriorLogisticR(x_c, m0, C0, D, targets)
			a = p_x/p_xc
			if a > 1
				x_samples[:,i] = x_c 
				xcur = x_c
...
# ╔═╡ 3bb73f86-eea9-490b-9fe6-bfd1d7a1b44c
x_samples = metropolis(false)

# ╔═╡ 21926728-cbe4-4412-b408-aa73476940c7
x_samples_gaussian_prior_proposal = metropolis(true)
