...
VI computes only the unnormalised distribution $p̃(\boldsymbol{x}) ≝ p(\boldsymbol{x}, D) = p^*(\boldsymbol{x})Z$. Since KL Divergence is always non-negative this means that we get the lower bound on the constant Z when optimising $J(q)$

2. what is evidence lower bound (ELBO) ?

Evidence Lower Bound (ELBO) - is the lower bound that is attained when optimising on KL Divergence. It is the lower bound on the log probability of the data $\log{p(D)}$.
...
Mean field VI assumes $q(w) = \prod_{i} q(w_i)$, i.e. the approximating variational distribution is fully independent. Then variational calculus tells us the best approximating distribution can be found by setting each dimension of $q$, $q(w_i)$ by the expectation of the log joint distribution: i.e.
...
Mean Field Approach assumes that the $q$ distribution can be factored by independent distributions. In Gibbs sampling, the markov assumption is made, where each sample is conditionally independent. Both make use of the Markov Blanket.
...
# ╔═╡ bd94146e-89db-44b3-8d7c-9caa3ccd8612
begin
	function svi_grad(mm, LL, Lsigma_w, neg_log_like_grad)
		D = size(mm)
		L = tril(LL)
		#diag = 
	end

	function logreg_negLlike(ww, X, yy)
		yy = 2*(yy==1) -1
		sigma = 1.0/(1.0 + exp.(-yy * (X * ww)))
