...
# ╔═╡ c1a7a83f-91a1-4a50-8ab1-72c7887fa97c
md"""
m₀ and C₀ are the mean and variance of the prior of the weights.
$p(\theta)= N(m_0, C_0),$
"""

# ╔═╡ e9260470-ee92-424f-a87c-87331e09ed8a
begin
	# Standard Logistic Regression log posterior, gradient, and Hessian. This is from previous week as well.
...
begin
	# This method is called once and will return all samples.
	function MCMCLogisticR(X, y, dim; mc= 1000, burnin=0, thinning=10, m₀= zeros(dim), C₀ = 100 .* Matrix(I, dim, dim), qV= nothing)
		# If a proposal distribution is not provided, then default to (Scott, 2009)
		# qV only sets the covariance of the Gaussian.
		if isnothing(qV)
			qV = Symmetric(inv(inv(C₀) + 6/π^2 * X' * X))
		end
		# m₀ and C₀ are the mean and covariance of the prior Gaussian?
		# This is some kind of proxy function of θ
	...
	# Believe this stands for: Metropolis Hastings Random Walk, Multivariate Normal
	# This method is called once and will return all samples.
	function MHRWMvN(pf, dim; logP = true, Σ = 10. *Matrix(I, dim, dim), x0=zeros(dim), mc=5000, burnin =0, thinning = 1)
		samples = zeros(dim, mc)
		# Effectively replaces having to sample from a MvN. Numerical stability and performance? Is this some kind of reparametrization?
		C = cholesky(Σ)
		L = C.L
		# q = MvNormal(x0, Σ)
		# pf is a function from the input. Given input vector x0 it will return the log posterior of the logistic regression function.
		pfx0 = pf(x0) 
		j = 1
		# Main MH loop. The number of loops adds burnin and multiplies by how much thinning. This reduces correlation and improves independece of the samples and increases ess "effective samples size".
		for i in 1:((mc+burnin)*thinning)
			# I think Cholesky avoids the below comments (sampling from MvN). Buy why avoid?
			# q = MvNormal(x0, Σ)
			# Applying/Multiplying L with rand will apply the covariance to the uniform sample. Then adding x0 will shift the samples to the mean of the gaussian.
			xstar = x0 + L * randn(dim)
			# xstar is the sampled weights. Given weights, the pf function returns the logistic regression log posterior.
			pfxstar = pf(xstar)
			# what is logP? LogPosterior? Looks like if log post, then it can perform subtraction. If not then it has to be division. exp is done to bring it back.
			if logP
				Acp = pfxstar - pfx0 
				Acp = exp(Acp)
			else
			...
			#rand will sample uniformly from [0,1], if the rand value is strictly lower than Acp boundary then the next state is the same as the current one. If rand is higher than Acp then x0 and pfx0 just remain untouched for t+1.
			if rand() < Acp
				x0 = xstar
				pfx0 = pfxstar
			end
			# The samples matrix is always populated by the accepted state as long as it is over the burnin threshold, and is the nth sample w.r.t thinning.
...
# ╔═╡ b19d1396-09b6-4d22-a920-24b93502a2f8
...
# ╔═╡ 68b2c055-5484-48e2-b9bb-12567a23809b
collect(x1)
...
begin
	...
	# This is not used, but appears to be a matrix form of the function below it.
	...
	#ws is the weights of multiple logistic regression models. This is performing the linear combination and logistic squashing for each model, and taking a mean. This is an ensemble prediction.
...
begin
	# Using the Gaussian prior as the proposal distribution isn't really a good idea.
...
## 2. Full Bayesian inference on hierarchical linear regression
