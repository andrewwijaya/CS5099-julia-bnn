...
- *numerical-stable,*
- *overfitting-proof,* 
- *and also more natural inference results.*
*We will use again linear regression and logistic regression as working examples.* 

You should read the following chapters. 
- Dynamic Linear Models with R by Petris et al.: Chapter 1.1 to 1.5
- MLAPP Chapter 7.5.1, 7.6 
- MLAPP Chapter 8.3.6, 8.4.1, 8.4.3, 8.4.4
"""
...
begin
	...
	function signal_(x, k= -0.2)
		# between -1 and 1 the signal is 1
		if (x> -1) && (x < 1) 
			return 1
		# between -8 and -7 the signal is something special
		elseif (x>-8) && (x<-7)
			return k * x + (1.6- k*(-8))
		# otherwise, just 0
		else
	...
	# not sure what exactly [1:N] accomplishes
...
# ╔═╡ 4ebb93ac-025a-4f70-9a79-735bcd0248f1
# I believe we convert R¹ to R⁵⁰ by applying RBF to each data point and each of the 50 centres
begin
	
	# Apply RBF to a single observation x based on parameters μ and σ
	function rbf(x, μ, σ)
		return exp(-(x - μ)^2/(2σ))
	end

	# Lei's vectorised RBF function
	function rbf_vectorised(x, μ, σ²)
		@. exp(-1 /(2 * σ²) * (x - μ')^2)
	end
	
	# numobs should be 201
	numobs = size(xs)[1]
	# X₅₀ is the RBF expanded design matrix, it is 201x50
	X₅₀ = zeros(numobs, 50)
	for i in range(1, numobs)
		for j in range(1, 50)
			# σ is fixed to 0.5
			X₅₀[i,j] = rbf(xs[i], xsamples[j], 0.5)
		end
	end

	# fit using MLE βᴹᴸᴱ = (XᵀX)⁻¹Xᵀy
	βᴹᴸᴱ = inv(transpose(X₅₀) * X₅₀) * transpose(X₅₀) * tₜₑₛₜ
	plot(X₅₀ * βᴹᴸᴱ, label="RBF", ylim= [-0.8, 2.5])
end

# ╔═╡ f3679749-74b9-4edf-8e7d-3e88b849800b
begin
	# Using Lei's vectorised RBF function
	#X₅₀_vectorised = zeros(numobs, 50)
	X₅₀_vectorised = rbf_vectorised(xsamples, xsamples, 0.5)
	n_obs = size(xsamples)[1]
	X₅₀_f = [ones(n_obs) X₅₀_vectorised]
	β_vᴹᴸᴱ = inv(transpose(X₅₀_f) * X₅₀_f) * transpose(X₅₀_f) * ts
	Φₜₑₛₜ = rbf_vectorised(xs, xsamples, 0.5)
	n_obs = size(xs)[1]
	Φₜₑₛₜf = [ones(n_obs) Φₜₑₛₜ]
	plot(Φₜₑₛₜf * β_vᴹᴸᴱ, label="RBF", ylim= [-0.8, 2.5])
end

# ╔═╡ 0d81a98e-cc03-4a65-b9cc-d9b668667a22
Φₜₑₛₜf

# ╔═╡ 51a3a0d1-74c5-4c40-88d2-901f7960f1ab
xsamples

# ╔═╡ 7d706fa3-ab98-41ce-9636-2a2a3938bd88
plot(xs, X₅₀, label="")

# ╔═╡ 07b07621-2fa8-40d2-9e7f-519e52485456
# xsamples is 50x1 and xs is 201x1
# xs is the design matrix R¹, we take RBF with centers from xsamples
ts

# ╔═╡ e312f1b7-7814-4ea8-871b-3d67fc602330
# Add zero mean gaussian prior P(θ) ~ N(0,1)
...
# ╔═╡ 4b0bea49-10de-4ec8-a471-a40825b89717
begin
