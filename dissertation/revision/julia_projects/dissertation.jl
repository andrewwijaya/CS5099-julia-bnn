...
# ╔═╡ 99e6c239-129d-4da4-a499-b9874a5da84d
begin
	using Flux
	using Distributions
	using Plots
...
# ╔═╡ 6443a4a0-fd6d-11ec-07f4-e952decb18b8
md"""
### Probabilistic Layers in Julia
"""

# ╔═╡ 08a81a57-c289-49c5-9882-c0520cdde167
md"""
#### Generate dataset
"""

# ╔═╡ 449f8d9d-3acb-41c4-b913-b4dd89d5ddbd
begin
	w0 = 0.125
	b0 = 5.
	x_range = [-20, 60]
...
# ╔═╡ 3a9eb0c8-84f2-4f89-ade3-495c3f0fc11e
y_train, x_train, x_tst = load_dataset()

# ╔═╡ c4080ecb-00b6-4848-89d9-855967c0a8a6
scatter(x_train, y_train, title="Synthetic Dataset", label="observations")

# ╔═╡ df4769fb-febd-4d3c-9842-c3c94531f872
md"""
#### Part 1: TFP Probabilistic Layers (No Uncertainty)
"""

# ╔═╡ fcf2b8c2-1868-4472-8a4e-0b8385651fcf
begin
	...
	function create_model()		
		#Simple model that assigns a Gaussian with mean parametrised by the output of the Dense layer and a variance of 1.
		model = Chain(
			#Logistic
			#Dense(1 => 2),
			#DistributionLambda((t)->Logistic(t[1], 1e-3 + softplus(0.05 * t[2])))
			
			#Laplace
			#Dense(1 => 2),
			#DistributionLambda((t)->Laplace(t[1], 1e-3 + softplus(0.05 * t[2])))

			#Gamma
			#Dense(1 => 2),
			#DistributionLambda((t)->Gamma(t[1], 1e-3 + softplus(0.05 * t[2])))

			#Poisson
			#Dense(1 => 2),
			#DistributionLambda((t)->Poisson(t[1], 1e-3 + softplus(0.05 * t[2])))
			
			#Cauchy
			#Dense(1 => 2),
			#DistributionLambda((t)->Cauchy(t[1], 1e-3 + softplus(0.05 * t[2])))
	...
	function train_step(model)
		...
		for (x, y) in train_loader
			...
			#TODO:Train is really slow for now
...
# ╔═╡ 4f425639-052f-47c1-bf40-eb6dbe5dfca9
x_train

# ╔═╡ a46876e0-fbae-4dbe-b540-6fe84a91140f
w0

# ╔═╡ fd6688c9-d8ab-4bd6-90f3-eebcb85a1586
b0

# ╔═╡ 051ab1a9-b11a-4022-8ef4-fd8bb87b312e
begin
	model = create_model()
	epochs = 100
	losses = zeros(epochs)
	anim = @animate for iter in 1:epochs
		losses[iter] = loss = train_step(model)
		gaussians = get_distributions(model, test_loader)
		means = get_means(gaussians)
		stddevs = get_stddevs(gaussians)
		scatter(x_train, y_train, label="observations", xlim=(-20,60), ylim=(-10, 30))
		plot!(x_tst, means, label="mean", linewidth=5, linecolor=:red, xlim=(-20,60), ylim=(-10, 30), title="Iteration: "*string(iter)*" Probabilistic Linear Regression")
		plot!(x_tst, means + 1 * stddevs, linewidth=5, linecolor=:purple, label="stdev+1", xlim=(-20,60), ylim=(-10, 30));
		plot!(x_tst, means + 2 * stddevs, linewidth=5, linecolor=:green, label="stdev+2", xlim=(-20,60), ylim=(-10, 30));
		plot!(x_tst, means - 2 * stddevs, linewidth=5, linecolor=:green, label="stdev-2", xlim=(-20,60), ylim=(-10, 30));
		plot!(x_tst, means - 1 * stddevs, linewidth=5, linecolor=:purple, label="stdev-1", xlim=(-20,60), ylim=(-10, 30));
	end
end

# ╔═╡ 25f87cd4-52f5-4103-9796-77a73dc75342
gif(anim, fps=20)

# ╔═╡ 92c60da2-3615-4a22-a11b-58e4d5a4bee6
Flux.params(model)

# ╔═╡ 0ac98873-798f-4d69-a61a-5b5c04a14c46
